...
def chatbot_response(msg):
    ...
    return res,tag
def eng_to_ur(eng_text):
    urdu_text = translator.translate(eng_text, dest="ur")
    return urdu_text.text
def ur_to_eng(urdu_text):
    eng_text= translator.translate(urdu_text, dest="en")
...
while True:
    ...
    if bboxs:
        if i==0:
            speak("السلام علیکم کیسے ہیں آپ")
            print("السلام علیکم کیسے ہیں آپ")
            i=i+1
        arduino.sendData([1])  
        urdu_text=takecommand()
        print(urdu_text)
        if urdu_text!=" ":
            user=ur_to_eng(urdu_text)
            print("user sentence:  ",user)
            response,tag=chatbot_response(user)
            print(response)
            urdu_text=eng_to_ur(response)
            print("response in urdu:  ",urdu_text)
            print(type(urdu_text))
            speak(urdu_text)
    if tag=="goodbye":
        break
    else:
